...
def _demo_actuals(n: int = 60) -> pd.DataFrame:
    ...
    return pd.DataFrame({'departure_date': dates, 'actual_price': prices})
...
def main():
    ...
    <div style="background:linear-gradient(90deg,#1a1a2e,#16213e,#0f3460);
      ...
      <p style="color:#aab;margin:6px 0 0 0;font-size:0.95rem">
    ...
    if not api_online:
        st.warning(
            f" Forecasting API not reachable at `{API_BASE}`.  "
            "Showing **demonstration data**.  "
            "Start the API with:  `uvicorn api:app --port 8000`",
            icon="⚠️")
    elif not db_ok:
        st.warning(
            "  API is running but the database is not yet populated.  "
    ...
    with st.sidebar:
        st.markdown(
            "<p style='color:#CC0000;font-size:1.8rem;font-weight:800;"
            "font-family:Georgia,serif;letter-spacing:1px;"
            "margin:0;padding:4px 0;'>Kenya Airways</p>"
        ...
        route    = st.selectbox("Select route", options=routes, index=0,
        ...
        sel_models = st.multiselect("Models to display",
                                     options=all_models,
                                     default=all_models,
                                     label_visibility='collapsed')
        ...
        history_days = st.slider("Actuals history (days)", 30, 180, 90)
        ...
        with st.expander(" Advanced Options"):
            ...
            st.caption(
                ...
                "The reliability heatmap identifies routes and booking contexts "
                "with elevated forecast error.  Analysts should apply additional "
                "scrutiny to pricing decisions in red-flagged cells."
            )
    ...
    # TAB LAYOUT
    # ═══════════════════════════════════════════════════════════════════════════
    tab_fc, tab_perf, tab_network, tab_custom, tab_pipeline = st.tabs([
        "📈 Forecasts",
        "📊 Test-Set Results",
        #"📈 Forecast Analysis",
    ...
    with tab_perf:
        ...
        dm_df = pd.DataFrame([
            ...
            {'Comparison':'Hybrid vs SA-LSTM',      'DM Stat':'+1.43', 'p-value':'0.156', 'Result':'No significant difference ⚖️'},
        ])
    ...
    with tab_custom:
        ...
        with col_in1:
            ...
            st.caption(
                ...
                "Minimum 35 values recommended for reliable forecasting."
            )
            default_prices = (
        ...
        if run_btn or True:   # always show — run button triggers re-computation
            ...
            if 'cust_fc_df' in st.session_state and not st.session_state['cust_fc_df'].empty:
                ...
                st.info(
                    "ℹ️ **Note:** The Standalone LSTM and Hybrid forecasts in this tab "
